...
class GameScreen(tk.Frame):
    ...
    def _switch(self):
        ...
        # Chaos mode: end the game once 50 moves have been made
        if (self.app.game_mode.get() == "Chaos") and (self.turn_count >= 50):
            self._end(); return
...
class ChaosManager:
    ...
    def chaos_click(self, e):
        ...
        if self.pending_event:
            # Mirror requires a valid move cell — reject invalid clicks so the
            # player can try again without silently losing their chaos event.
            if self.pending_event.name == "Mirror":
                if (r, c) not in valid_moves(self.board, BLACK):
                    messagebox.showwarning("Invalid Cell",
                        "Mirror requires a valid move cell. Try again.")
                    return

            player = BLACK  # player events are always the human player
            event_name = self.pending_event.name
            event_side = self.pending_side
            ...
            self.log_event(event_name, event_side)
            self.pending_event      = None
            self.pending_side       = None
            self.pending_flippa_mode = None
            if self.r is None:
